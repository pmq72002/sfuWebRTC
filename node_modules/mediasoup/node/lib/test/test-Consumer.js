...
beforeEach(async () => {
    ...
    ctx.audioProducer = await ctx.transport1.produce(ctx.audioProducerParameters);
    ctx.videoProducer = await ctx.transport1.produce(ctx.videoProducerParameters);
});
afterEach(() => {
    ctx.worker?.close();
});
test('transport.consume() succeeds', async () => {
    ...
    ctx.transport2.observer.once('newconsumer', onObserverNewConsumer1);
    ...
    expect(audioConsumer.score).toEqual({ score: 10, producerScore: 0, producerScores: [0] });
    ...
    ctx.transport2.observer.once('newconsumer', onObserverNewConsumer2);
    expect(ctx.router.canConsume({
    ...
    expect(videoConsumer.score).toEqual({ score: 10, producerScore: 0, producerScores: [0, 0, 0, 0] });
    ...
    ctx.transport2.observer.once('newconsumer', onObserverNewConsumer3);
    expect(ctx.router.canConsume({
        producerId: ctx.videoProducer.id,
        rtpCapabilities: ctx.consumerDeviceCapabilities
    }))
    ...
    expect(videoPipeConsumer.score).toEqual({ score: 10, producerScore: 10, producerScores: [0, 0, 0, 0] });
    ...
    expect(dump2.mapProducerIdConsumerIds)
        ...
        },
        {
            ...
            values: expect.arrayContaining([videoConsumer.id, videoPipeConsumer.id])
        }
    ]));
...
test('transport.consume() can be created with user provided mid', async () => {
    ...
    expect(audioConsumer1.rtpParameters.mid).toEqual(expect.stringMatching(/^[0-9]+/));
    const audioConsumer2 = await ctx.transport2.consume({
        producerId: ctx.audioProducer.id,
        mid: 'custom-mid',
        rtpCapabilities: ctx.consumerDeviceCapabilities
    ...
    expect(audioConsumer3.rtpParameters.mid).toEqual(expect.stringMatching(/^[0-9]+/));
    expect(Number(audioConsumer1.rtpParameters.mid) + 1).toBe(Number(audioConsumer3.rtpParameters.mid));
}, 2000);
...
test('Consumer emits "score"', async () => {
    ...
    const notificationOffset = notification_1.Notification.createNotification(builder, builder.createString(audioConsumer.id), notification_1.Event.CONSUMER_SCORE, notification_1.Body.Consumer_ScoreNotification, consumerScoreNotification.pack(builder));
    ...
    channel.emit(audioConsumer.id, notification_1.Event.CONSUMER_SCORE, notification);
    channel.emit(audioConsumer.id, notification_1.Event.CONSUMER_SCORE, notification);
    channel.emit(audioConsumer.id, notification_1.Event.CONSUMER_SCORE, notification);
    expect(onScore).toHaveBeenCalledTimes(3);
    expect(audioConsumer.score).toEqual({ score: 9, producerScore: 10, producerScores: [8] });
}, 2000);
